...
lemma dia_inv [Group₁ G] (a : G) : a ⋄ a⁻¹ = 𝟙 := by
  ...
  have h': (a⁻¹)⁻¹ = a := by rw [inv_eq_of_dia]; exact h
  nth_rewrite 1 [<-h']
  apply inv_dia
...
lemma inv_eq_of_mul [Group₃ G] {a b : G} (h : a * b = 1) : a⁻¹ = b := by
  have h': a⁻¹ * a = 1 := by apply Group₃.inv_mul
...
lemma mul_right_cancel₃ {G : Type} [Group₃ G] {a b c : G} (h : b*a = c*a) : b = c := by
  have h' : (a⁻¹)⁻¹ = a := by
    apply inv_eq_of_mul (Group₃.inv_mul a)
  rw [<-mul_one b, <-(Group₃.inv_mul a⁻¹), h', <-mul_assoc₃, h, mul_assoc₃, Group₃.mul_inv, mul_one]
