...
@[simp]
theorem zero_re : (0 : GaussInt).re = 0 := by
  rfl
...
theorem sq_add_sq_eq_zero {α : Type*} [LinearOrderedRing α] (x y : α) :
    ...
    have hx : x ^ 2 ≥ 0 := by apply sq_nonneg
    have hy : y ^ 2 ≥ 0 := by apply sq_nonneg
    rw [add_eq_zero_iff_of_nonneg hx hy]
    constructor
    · intro h
      constructor
      · apply pow_eq_zero h.1
      · apply pow_eq_zero h.2
    · intro h
      constructor
      · rw [h.1]
        norm_num
...
theorem norm_nonneg (x : GaussInt) : 0 ≤ norm x := by
  rw [norm]
  have hre : x.re ^ 2 ≥ 0 := by exact sq_nonneg x.re
  have him : x.im ^ 2 ≥ 0 := by exact sq_nonneg x.im
  calc
    0 ≤ 0 + 0 := by linarith
    _ ≤ x.re ^ 2 + 0 := by
      exact (Int.add_le_add_iff_right 0).mpr hre
    _ ≤ x.re ^ 2 + x.im ^ 2 := by
      exact Int.add_le_add_left him (x.re ^ 2)
