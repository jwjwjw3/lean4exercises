...
theorem primes_infinite : ∀ n, ∃ p > n, Nat.Prime p := by
  ...
  have : 2 ≤ Nat.factorial (n + 1) + 1 := by
    ...
    · norm_num
    calc
      2 ≤ (n+1).factorial + 1 := by exact ih
      _ ≤ (n+1)*(n+1).factorial + (n+1).factorial + 1 := by
        exact Nat.le_add_left ((n + 1).factorial + 1) ((n + 1) * (n + 1).factorial)
      _ = (n+1+1) * (n+1).factorial + 1 := by ring
      _ = (n + 1 + 1).factorial + 1 := by
        exact rfl
  ...
  have : p ∣ Nat.factorial (n + 1) := by
    ...
    exact Nat.le_add_right_of_le ple
  have : p ∣ 1 := by
