...
def conjugate {G : Type*} [Group G] (x : G) (H : Subgroup G) : Subgroup G where
  ...
  inv_mem' := by
    dsimp
    intro y hp
    rcases hp with ⟨h, hprops⟩
    use h⁻¹
    constructor
    · apply (Subgroup.inv_mem_iff H).mpr hprops.1
    · rw [hprops.2]
      rw [conj_inv]
  mul_mem' := by
    dsimp
...
lemma eq_bot_iff_card {G : Type*} [Group G] {H : Subgroup G} :
  ...
  · intro h
    intro x xH
    rcases h with ⟨x', _, aHx'⟩
    calc
      x = x' := by apply aHx' x xH
      _ = 1 := by apply (aHx' 1 H.one_mem).symm

-- #check card_dvd_of_le

lemma inf_bot_of_coprime {G : Type*} [Group G] (H K : Subgroup G)
    (h : (Nat.card H).Coprime (Nat.card K)) : H ⊓ K = ⊥ := by
  have D₁ : Nat.card (H ⊓ K : Subgroup G) ∣ Nat.card H := by
    apply card_dvd_of_le inf_le_left
  have D₂ : Nat.card (H ⊓ K : Subgroup G) ∣ Nat.card K := by
    apply card_dvd_of_le inf_le_right
...
-- perm group multiplication from right to left
...
lemma aux_card_eq [Finite G] (h' : Nat.card G = Nat.card H * Nat.card K) :
  ...
  have h3 : Nat.card G = H.index * Nat.card H := by
    exact Eq.symm (index_mul_card H)
  have h4 : H.index * Nat.card H = Nat.card ↥K * Nat.card ↥H := by
    rw [h3] at h'
    nth_rewrite 2 [mul_comm] at h'
    exact h'
  apply Nat.eq_of_mul_eq_mul_right h2 h4
...
def iso₂ : G ≃* (G ⧸ K) × (G ⧸ H) := by
  ...
  · rw [<-ker_eq_bot_iff]
    ...
    simp [h.symm.eq_bot]
  · rw [Nat.card_prod, h']
    have h'' : Nat.card G = Nat.card ↥K * Nat.card ↥H := by
      rw [mul_comm] at h'
      exact h'
    rw [aux_card_eq h']
    rw [aux_card_eq h'']
...
def finalIso : G ≃* H × K := by
  have h1 : K ≃* G ⧸ H := by
    apply iso₁ h h'
  have h's : Nat.card G = Nat.card ↥K * Nat.card ↥H := by
    rw [mul_comm] at h'
    exact h'
  have hs : Disjoint K H := by
    exact _root_.id (Disjoint.symm h)
  have h2 : H ≃* G ⧸ K := by
    apply iso₁ hs h's
  have h3 : G ≃* (G ⧸ K) × (G ⧸ H) := by
    apply iso₂ h h'
  apply h3.trans
  apply MulEquiv.prodCongr h2.symm h1.symm
