...
def chineseMap (I : ι → Ideal R) : (R ⧸ ⨅ i, I i) →+* Π i, R ⧸ I i := by
  apply Ideal.Quotient.lift (⨅ i, I i) (Pi.ringHom fun i : ι ↦ Ideal.Quotient.mk (I i))
...
theorem isCoprime_Inf {I : Ideal R} {J : ι → Ideal R} {s : Finset ι}
  ...
  | @insert i s _ hs =>
      ...
      calc
        1 = I + K                  := by
          symm
          -- apply (hs fun j hj ↦ hf j (Finset.mem_insert_of_mem hj))
          apply hs
          intro j hj
          apply Finset.mem_insert_of_mem at hj
          apply hf
          exact hj
